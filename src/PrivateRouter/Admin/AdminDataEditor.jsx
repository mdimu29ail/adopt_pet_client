...
import useAxiosSecure from '../../hooks/useAxiosSecure';
import useUserRole from '../../hooks/useUserRole';
// আপনার supabase client ফাইলটি ঠিক যে ডিরেক্টরিতে আছে, সেই অনুযায়ী ইমপোর্ট পাথটি পরিবর্তন করে নিন
...
const AdminDataEditor = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // ১. Supabase থেকে Users ডাটা ফেচ করা
      const { data: supabaseUsers, error: usersError } = await supabase
        .from('users') // আপনার Supabase-এর টেবিলের নাম যদি ভিন্ন হয়, তবে এখানে পরিবর্তন করুন
        .select('*');
      ...
      // ২. Axios দিয়ে বাকি ডাটা ফেচ করা
      const [petsRes, adoptionsRes, campaignsRes, paymentsRes] =
        await Promise.all([
          axiosSecure.get('/admin/pets'),
      ...
      // ৩. স্টেট আপডেট করা
      setData({
        users: supabaseUsers || [], // Supabase থেকে আসা ডাটা
        pets: petsRes.data || [],
        adoptions: adoptionsRes.data || [],
        campaigns: campaignsRes.data || [],
        payments: paymentsRes.data || [],
      ...
      Swal.fire({
        icon: 'error',
        title: 'Sync Failed',
        text: 'Could not load admin data from the server or Supabase.',
        confirmButtonColor: '#37948b',
      });
    } finally {
      setLoading(false);
                    ...
                    {data[activeTab].length === 0 ? (
                      ...
                      data[activeTab].map((item, idx) => (
                        ...
                        >
                          ...
                          {/* Pets, Adoptions, Campaigns, Payments Row same as before */}
                          {activeTab === 'pets' && (
                            <>
                              <td className="p-6">
                                <div className="flex items-center gap-4">
