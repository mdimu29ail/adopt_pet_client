...
import Swal from 'sweetalert2';
import useAuth from '../hooks/useAuth';
// import useAxios from '../hooks/useAxios'; // অব্যবহৃত থাকায় রিমুভ করা হয়েছে
import Logo from '../Components/Logo/Logo';
...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // কিছু OAuth মেথড রিডাইরেক্ট করে, যদি পপআপ হয় তবে নিচে হ্যান্ডেল হবে:
      Swal.fire({
        icon: 'success',
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <motion.button
              ...
              type="submit"
              disabled={loading}
              className="w-full py-4 bg-[#37948b] text-white font-black rounded-2xl shadow-xl shadow-[#37948b33] transition-all flex items-center justify-center gap-3 text-lg disabled:opacity-60 cursor-pointer"
            >
              {loading ? (
          ...
          <motion.button
            ...
            className="w-full flex items-center justify-center gap-4 py-4 bg-white border-2 border-gray-100 rounded-2xl font-bold text-gray-700 hover:border-[#37948b] transition-all disabled:opacity-60 cursor-pointer"
          >
            {googleLoading ? (
              <FaSpinner className="animate-spin text-xl text-[#37948b]" />
            ) : (
              <>
                <FaGoogle className="text-red-500 text-xl" />
                <span>Continue with Google</span>
